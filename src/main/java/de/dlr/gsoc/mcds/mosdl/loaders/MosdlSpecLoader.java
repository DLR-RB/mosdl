...
/**
 * Specification loader to load an MO service specification from a set of MOSDL files.
 * <p>
 * All MOSDL files comprising a single specification need to be supplied in the call to
 * {@link #setInput(File...)}. MOSDL files need to have '.mosdl' file ending and have to conform to
 * the MO service description language. Alternatively, directories containing MOSDL files may be
 * supplied.
 */
public class MosdlSpecLoader implements SpecLoader {
	 ...
	 *
	 * @param isLaxMode {@code true} to try to recover from errors, {@code false} to always fail on
	 * errors
	 */
	...
	/**
	 * Sets the input files or directories that comprise a single MO service specification in MOSDL
	 * format.
	 * <p>
	 * @param input a set of MOSDL files that comprise an MO service specification. Alternatively,
	 * directories containing MOSDL files.
	 */
	@Override
	...
	private static class ErrorListener extends BaseErrorListener {
		...
		public void syntaxError(Recognizer<?, ?> recognizer, Object offendingSymbol, int line, int charPositionInLine, String msg, RecognitionException e) {
			String errorMsg = String.format("Syntax error in %s:%d:%d - %s", recognizer.getInputStream().getSourceName(), line, charPositionInLine, msg);
			if (isLaxMode) {
	...
	/**
	 * Class to listen for parser events from which the service specification is built successively.
	 */
	private static class ParseListener extends MOSDLBaseListener {
		...
		private static final String[] MAL_FUNDAMENTALS = {"Blob", "Boolean", "Double", "Duration", "FineTime", "Float", "Identifier",
		 ...
		 *
		 * @param isLaxMode {@code true} to try to recover from errors, {@code false} to always fail
		 * on errors
		 * @param spec the specification to add the new specification created from the parser events
		 * to
		 */
		...
		/**
		 * Automatically import the fundamental MAL data types so they can be referred to by
		 * unqualified name.
		 */
		 ...
		 * <p>
		 * This mechanism allows the user to omit identification numbers and have them
		 * auto-generated. Counters are initialized to the highest numbers already present in the
		 * spec so that any user-set numbers are not overwritten.
		 */
		...
		public void enterService(MOSDLParser.ServiceContext ctx) {
			...
			currentService.setComment(getDoc(ctx.doc()));
		...
		/**
		 * Gets the number identifier from a terminal node or create new one in case no number has
		 * been specified.
		 * <p>
		 * In any case, the current counter is incremented so that it can supply a new valid number
		 * identifier when called again.
		 *
		 * @param number the terminal node containing the user-specified number or {@code null} if
		 * the user did not specify a number
		 * @param counter the counter to use for determining the next number used as identifier
		 ...
		 * <p>
		 * If no number has been specified, {@code null} is returned. This method does not have side
		 * effects.
		 *
		 * @param nvidentifier the 'nvidentifier' parse context
		 * @return the contained number or {@code null} if no number has been given
		 */
		...
		/**
		 * Gets the version specified in the 'nvidentifier' parse context or the default version if
		 * none was specified.
		 *
		 * @param nvidentifier the 'nvidentifier' parse context
		...
		private void conditionallyFailOnError(String msg, ParserRuleContext context) {
			String errorMsg = String.format("Error in %s:%d:%d - %s", context.getStart().getInputStream().getSourceName(), context.getStart().getLine(), context.getStart().getCharPositionInLine(), msg);
			if (isLaxMode) {
				errorMsg += " Trying to recover.";
			}
			logger.error(errorMsg);
	 ...
	 * <p>
	 * Documentation for messages and message body parts can occur in two places: In 'bulk' using
	 * special tags for the operation documentation (similar to Javadoc) or right before each
	 * message and message body part (referred to as 'local doc' in this class). This class is
	 * responsible for parsing 'bulk' documentation and providing the final documentation by
	...
	private static class OperationDoc {
		...
		public OperationDoc(final String doc) {
			...
			String[] lines = doc.split("(^|\\r?\\n)\\s*@"); // split at line breaks (or beginning of string) followed by arbitrary white space followed by @
			if (lines.length == 0) {
				opDoc = null;
				return;
			}
			...
			for (; i < lines.length; i++) {
				String line = lines[i];
				String[] tagSplit = line.split(":\\s", 2); // a tag consists of a name, an optional parameter and a colon followed by whitespace, the tag value comes next
		 ...
		 * @param localDoc any local documentation for the message, may be {@code null}
		 ...
		 * @param localDoc any local documentation for the message body part, may be {@code null}
		 ...
		 * @param localDoc any local documentation for the error, may be {@code null}
		...
		/**
		 * Gets the documentation for the extra information of an error raised by the operation.
		 *
		 * @param errorName name of the error
		 * @param localDoc any local documentation for the extra information of the error
		 * @return the complete documentation for the extra information of the error
		 */
